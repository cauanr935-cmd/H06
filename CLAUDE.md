...
| `APP_URL` | Vercel + `.env.local` | base da LP, sem barra final; usada para montar `completed_redirect_url`. Dev: `http://localhost:3000`. Prod: domínio da Vercel |
...
**Deploy:** LP publicada em produção na Vercel (`https://h06.vercel.app`) — todas as env vars cadastradas lá (`DOCUSEAL_*`, `SUBMITTER_ROLE`, `APP_URL=https://h06.vercel.app`, `WEBHOOK_SECRET`, `UPSTASH_REDIS_REST_URL`/`TOKEN`), confirmado via `curl` (saiu de 500 `server_misconfigured` pros erros de validação esperados). **`h06.online` (domínio raiz) ainda não está conectado** — hoje é só a página de "domínio estacionado" da Hostinger; a LP só existe em `h06.vercel.app` por enquanto. Commits `b8466e2` e `8a21a2f` (Fases A-E + idempotência) já empurrados pro `origin/main`.

**Pendente:**
- Conectar o domínio `h06.online` (ou um subdomínio, ex. `app.h06.online`) ao projeto na Vercel, se for esse o plano — hoje só `h06.vercel.app` está no ar
